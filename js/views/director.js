...
export async function addDirectorComment(projectId) {
    ...
    try {
        const comment = {
            ...
            text: text,
            date: firebaseApp.firestore.Timestamp.now(), // ✅ Usar Timestamp de Firebase
            role: 'director'
        };

        await db.collection("collaborativeProjects").doc(projectId).update({
            directorComments: firebaseApp.firestore.FieldValue.arrayUnion(comment) // ✅ Usar FieldValue de Firebase
        });
...
export async function deleteDirectorComment(projectId, commentTimestamp) {
    ...
    try {
        ...
        await db.collection("collaborativeProjects").doc(projectId).update({
            directorComments: firebaseApp.firestore.FieldValue.arrayRemove(commentToDelete) // ✅ Usar FieldValue de Firebase
        });
